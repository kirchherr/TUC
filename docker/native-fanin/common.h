...
static int emit(const char *mode, const char *reason, bool security, unsigned int passed,
  ...
  residency_emit();
  printf("{\"schema_version\":\"tuc.bounded_fanin_numeric.v0\","
         "\"target\":\"%s\",\"mode\":\"%s\",\"status\":\"%s\",\"reason_code\":\"%s\","
         "\"source_intent_digest\":\"%s\",\"code_digest\":\"%s\",\"corpus_digest\":\"%s\","
         "\"contract_digest\":\"%s\",\"operation_order\":[\"relu_left\",\"relu_right\",\"matmul\",\"sum_axis1\"],"
         "\"output_names\":[\"row_sum\"],\"output_shapes\":[[33]],\"output_values\":%u,\"case_count\":%u,\"cases_passed\":%u,\"failed_run_index\":%d,"
