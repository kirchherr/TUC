...
#define host_relu_left tuc_relu_left
#define host_relu_right tuc_relu_right
#define host_sum tuc_sum_axis1
static bool target_ready(void) { return active->mask == 0U && schedule_valid() && sizeof(float) == 4U; }
static bool allocate_buffer(float **out, unsigned int bytes, unsigned int space) {
  if (space != 0U) return false;
  *out = malloc(bytes); return *out != NULL;
}
