...
static int emit(const char *mode, const char *reason, bool security, unsigned int passed,
  ...
  residency_emit();
  printf("{\"schema_version\":\"tuc.bounded_placement_numeric.v0\","
         ...
         "\"output_shape\":[%u],\"case_count\":%u,\"cases_passed\":%u,\"failed_run_index\":%d,"
         ...
         TUC_CORPUS_DIGEST, TUC_CONTRACT_DIGEST, TUC_ROWS, TUC_CASES, passed, failed,
         calls, passed * TUC_ROWS, bytes, rounded, security ? "true" : "false",
...
int main(int argc, char **argv) {
  ...
  float output[TUC_ROWS] = {0}, first[TUC_ROWS] = {0};
