...
python3 examples/bounded_native_io.py > /dev/null
umask 077
mkdir -p tmp
evidence_dir=$(mktemp -d "tmp/native-io-$target.XXXXXXXX")
container_name="tuc-io-$target-$$"
image_name="tuc-io-$target-$$"
cleanup() { docker rm -f "$container_name" > /dev/null 2>&1 || true; }
trap cleanup EXIT
trap 'exit 1' HUP INT TERM
timeout 600s docker build --network=none --target "$target-runtime" \
    -t "$image_name:runtime" -f docker/native-io/Dockerfile .
...
python3 examples/bounded_native_io.py --target "$target" --preflight \
    --validate "$evidence_dir/preflight.json"
run_worker "$image_id" /opt/tuc/proof --execute > "$evidence_dir/execution.json"
python3 examples/bounded_native_io.py --target "$target" \
    --validate "$evidence_dir/execution.json" --image-id "$image_id" > "$evidence_dir/record.json"
for mutation in bypass-relu late-relu missing-sum wrong-stride incomplete-coverage over-budget nonfinite unknown-opcode wrong-order input-overwrite missing-step wrong-target io-count io-direction io-size io-slot io-early-output io-duplicate-input io-short-output io-skip-output; do
    result=0
...
if [ "$target" = c11 ]; then
    timeout 600s docker build --network=none --target c11-sanitizer \
        -t "$image_name:sanitizer" -f docker/native-io/Dockerfile .
    ...
    python3 examples/bounded_native_io.py --target c11 --validate "$evidence_dir/sanitized.json"
fi
printf 'Plan-driven native I/O %s: PASS, 10 vectors + replay, twenty negative probes rejected.\n' "$target"
