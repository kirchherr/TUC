...
compile /out/proof
compile /out/schedule-fanout -DTUC_SCHEDULE_FAULT=8
compile /out/schedule-output -DTUC_SCHEDULE_FAULT=9
compile /out/skip-second-publish -DTUC_SKIP_SECOND_PUBLISH=1
compile /out/invalidate-shared -DTUC_INVALIDATE_SHARED=1
compile /out/clobber-shared -DTUC_CLOBBER_SHARED=1
...
compile /out/schedule-placement -DTUC_SCHEDULE_FAULT=7
...
sed 's/value < 0.0F ? 0.0F : value/value/' reviewed.cuh > kernels.cuh
compile /out/bypass-relu
sed 's/value < 0.0F ? 0.0F : value/value/;s/output\[row\] = value;/output[row] = value < 0.0F ? 0.0F : value;/' reviewed.c > generated.c
sed 's/value < 0.0F ? 0.0F : value/value/;s/output\[row\] = value;/output[row] = value < 0.0F ? 0.0F : value;/' reviewed.cuh > kernels.cuh
compile /out/late-relu
sed 's/value += projection\[row \* 5U + column\];/value = projection[row * 5U + column];/' reviewed.c > generated.c
sed 's/value += projection\[row \* 5U + column\];/value = projection[row * 5U + column];/' reviewed.cuh > kernels.cuh
compile /out/missing-sum
sed 's/b\[inner \* 5U + column\]/b[inner + column]/' reviewed.c > generated.c
sed 's/b\[inner \* 5U + column\]/b[inner + column]/' reviewed.cuh > kernels.cuh
compile /out/wrong-stride
sed 's/index < 165U/index < 164U/' reviewed.c > generated.c
sed 's/index < 165U/index < 164U/' reviewed.cuh > kernels.cuh
compile /out/incomplete-coverage
