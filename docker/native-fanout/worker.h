#ifndef TUC_FANOUT_WORKER_H
#define TUC_FANOUT_WORKER_H
...
static bool evaluate(const float *a, const float *b, float *output, unsigned int *calls) {
  ...
  for (unsigned int i = 0U; i < TUC_BUFFER_COUNT; ++i) {
    ...
    if (slot.space == 0U && slot.tensor < 2U) read_slots[i] = NULL;
      ...
      if (slot.space == 0U && slot.tensor >= 4U) {
        write_slots[i] = output + (slot.tensor - 4U) * TUC_ROWS;
  ...
  for (unsigned int i = 0U; i < TUC_RESIDENCY_COUNT; ++i) {
    ...
    if (s.kind == 0U) {
      ...
      if (out.tensor == 2U) ++projection_copy_calls;
      ...
      if (s.space == 0U) {
        switch (s.opcode) {
          ...
          case 3U: case 4U: host_sum(read_slots[s.a], write_slots[s.out]); break;
      ...
      if (s.opcode == 2U || s.opcode == 3U) ++shared_consumer_calls;
#ifdef TUC_INVALIDATE_SHARED
      if (s.opcode == 2U) available[s.a] = false;
#endif
#ifdef TUC_CLOBBER_SHARED
      if (s.opcode == 2U && !poison_buffer(write_slots[s.a], buffer_at(s.a).bytes, s.space)) return false;
#endif
    } else {
#ifdef TUC_SKIP_PUBLISH
      continue;
#endif
#ifdef TUC_SKIP_SECOND_PUBLISH
      if (buffer_at(s.a).tensor == 5U) continue;
#endif
      const unsigned int tensor = buffer_at(s.a).tensor;
      if (!available[s.a] || read_slots[s.a] != output + (tensor - 4U) * TUC_ROWS) return false;
      ++published_outputs;
    }
