#ifndef TUC_SHAPE_COMMON_H
#define TUC_SHAPE_COMMON_H
#include "inputs.h"
#include <math.h>
#include <stdbool.h>
#include <stdio.h>
...
static bool reference_matches(unsigned int index, const float *output) {
  /* Contract B first in binary64, independently of the emitted matmul loops. */
  for (unsigned int row = 0; row < TUC_ROWS; ++row) {
    double reference = 0.0;
    for (unsigned int k = 0; k < TUC_INNER; ++k) {
      double sum = 0.0;
      for (unsigned int c = 0; c < TUC_COLUMNS; ++c)
        sum += (double)TUC_B[index][k * TUC_COLUMNS + c];
      reference += (double)TUC_A[index][row * TUC_INNER + k] * sum;
    }
...
static int emit(const char *mode, const char *reason, bool security,
  ...
  const bool correct = ok && strcmp(mode, "execute") == 0 && passed == TUC_RUNS;
  printf("{\"schema_version\":\"tuc.bounded_reduction_shape_observation.v0\","
         "\"target\":\"%s\",\"mode\":\"%s\",\"status\":\"%s\",\"reason_code\":\"%s\","
         "\"source_intent_digest\":\"%s\",\"code_digest\":\"%s\",\"corpus_digest\":\"%s\","
         "\"output_shape\":[%u],\"security_boundary_passed\":%s,"
         "\"case_count\":%u,\"cases_passed\":%u,\"failed_run_index\":%d,"
         "\"generated_function_calls\":%u,\"tensor_bytes\":%u,"
         "\"reference_correctness\":%s,\"repeated_baseline_passed\":%s,"
         "\"raw_values_serialized\":false}\n",
         TUC_TARGET, mode, ok ? "PASS" : "ERROR", reason, TUC_INTENT_DIGEST,
         TUC_CODE_DIGEST, TUC_CORPUS_DIGEST, TUC_ROWS, security ? "true" : "false",
         TUC_CASES, passed, failed, calls, bytes, correct ? "true" : "false",
         correct ? "true" : "false");
  return ok ? 0 : 1;
}
...
int main(int argc, char **argv) {
  ...
  return emit(mode, reason, true, passed, failed, calls, TUC_TENSOR_BYTES);
}
#endif
