...
static bool evaluate(const float *a, const float *b, float *output, unsigned int *calls) {
  ...
  /* All-ones FP32 is NaN; every logical output must be overwritten. */
#ifdef TUC_INCOMPLETE_COVERAGE
  constexpr unsigned int projection_blocks = 1U, output_blocks = 1U;
#else
  constexpr unsigned int projection_blocks = TUC_PROJECTION_BLOCKS;
  constexpr unsigned int output_blocks = TUC_OUTPUT_BLOCKS;
#endif
  tuc_projection<<<projection_blocks, 32>>>(buffers[0], buffers[1], buffers[2]);
  ++*calls;
  if (cudaGetLastError() != cudaSuccess || cudaDeviceSynchronize() != cudaSuccess) return false;
  tuc_sum_axis1<<<output_blocks, 32>>>(buffers[2], buffers[3]);
  ++*calls;
  if (cudaGetLastError() != cudaSuccess || cudaDeviceSynchronize() != cudaSuccess) return false;
