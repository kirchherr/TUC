"""Index Runtime Execution Output Closure for Kernel Ingress research cases."""
...
SOURCE_TO_INTENT_RESEARCH_KERNEL_INGRESS_RUNTIME_OUTPUT_CLOSURE_INDEX_REPORT_SCHEMA_VERSION = (
    "tuc.source_to_intent_research_kernel_ingress_runtime_output_closure_index_report.v0"
)
...
SOURCE_TO_INTENT_RESEARCH_KERNEL_INGRESS_RUNTIME_OUTPUT_CLOSURE_INDEX_ARTIFACT_POLICY = (
    "digest_only_values_omitted"
)
SOURCE_TO_INTENT_RESEARCH_KERNEL_INGRESS_RUNTIME_OUTPUT_CLOSURE_INDEX_SOURCE_BOUNDARY = (
    "kernel_ingress_runtime_to_output_closure_index"
)
SOURCE_TO_INTENT_RESEARCH_KERNEL_INGRESS_RUNTIME_OUTPUT_CLOSURE_INDEX_FORBIDDEN_FRAGMENTS = (
    "@triton.jit",
    "import triton",
    '"module_source":',
    "python_source",
...
_TOP_LEVEL_KEYS = frozenset(
    {
        ...
        "output_closure_policy_id",
        "output_closure_required_evidence_kinds",
        "output_closure_schema_version",
...
def build_kernel_ingress_runtime_output_closure_index_report() -> dict[str, object]:
    """Return source-free digest bindings for Kernel Ingress output closures."""
    ...
    report: dict[str, object] = {
        ...
        "output_closure_policy_id": RUNTIME_EXECUTION_OUTPUT_CLOSURE_POLICY_ID,
        "output_closure_required_evidence_kinds": list(
            RUNTIME_EXECUTION_OUTPUT_CLOSURE_REQUIRED_EVIDENCE_KINDS
        ),
        "output_closure_schema_version": (
            RUNTIME_EXECUTION_OUTPUT_CLOSURE_REPORT_SCHEMA_VERSION
        ),
        "output_closure_status": RUNTIME_EXECUTION_OUTPUT_CLOSURE_STATUS,
...
def build_report() -> str:
    """Return stable JSON evidence for the Kernel Ingress output closure index."""
...
def assert_kernel_ingress_runtime_output_closure_index_report_contract(
    report: object,
) -> None:
    """Fail closed unless the Kernel Ingress output closure index matches v0."""

    if not isinstance(report, Mapping):
        raise ValueError("kernel ingress runtime output closure index must be object")
    ...
    for key, expected in expected_values.items():
        if report[key] != expected:
            raise ValueError(f"kernel ingress runtime output closure index {key} drift")
    for key in (
        "runtime_evidence_bundle_index_digest",
        "runtime_matrix_digest",
        "runtime_step_trace_digest",
    ...
    if not isinstance(cases, list):
        raise ValueError("kernel ingress runtime output closure index cases drift")
    observed_case_ids = []
    for case in cases:
        observed_case_ids.append(_assert_case_contract(case))
    if tuple(observed_case_ids) != tuple(_EXPECTED_CASES):
        raise ValueError("kernel ingress runtime output closure index case order drift")
...
def _build_case(
    ...
    bundle_report_digest = _digest(dump_runtime_execution_evidence_bundle_report(bundle))
    ...
    if receipt.receipt_metadata_digest != evidence_bundle_case["execution_receipt_digest"]:
        raise ValueError("kernel ingress output closure receipt digest drift")
    ...
    return {
        ...
        "bundle_report_digest": bundle_report_digest,
        "case_id": case_id,
        "execution_receipt_digest": receipt.receipt_metadata_digest,
        "graph_name": closure.graph_name,
        "kernel_name": kernel_name,
        "operation_path": list(step_trace_case["operation_path"]),
        "output_closure_check_count": closure.check_count,
        "output_closure_contract": closure.closure_contract,
        "output_closure_metadata_digest": closure.closure_metadata_digest,
        "output_closure_policy_id": closure.closure_policy_id,
        "output_closure_report_digest": _digest(closure_text),
        "output_closure_status": closure.closure_status,
        "output_count": closure.source_public_output_bundle_item_count,
        "passed": closure.passed,
        "public_output_bundle_digest": (
            closure.source_public_output_bundle_metadata_digest
        ),
        "raw_value_policy": closure.raw_value_policy,
        "source_output_contract_digest": (
            closure.source_output_contract_metadata_digest
        ),
        "source_output_contract_item_count": (
            closure.source_output_contract_item_count
        ),
        "source_public_output_bundle_item_count": (
            closure.source_public_output_bundle_item_count
        ),
...
def _case_by_id(report: Mapping[object, object], case_id: str) -> Mapping[object, object]:
    cases = report["cases"]
    if not isinstance(cases, list):
        raise ValueError("kernel ingress runtime output closure source cases drift")
    matches = [
        case
        for case in cases
    ...
    if len(matches) != 1:
        raise ValueError("kernel ingress runtime output closure source case drift")
    return matches[0]


def _assert_case_contract(case: object) -> str:
    if not isinstance(case, Mapping):
        raise ValueError("kernel ingress runtime output closure case must be object")
    _assert_exact_keys("case", case, _CASE_KEYS)
    case_id = case["case_id"]
    if not isinstance(case_id, str) or case_id not in _EXPECTED_CASES:
        raise ValueError("kernel ingress runtime output closure case id drift")
    expected = _EXPECTED_CASES[case_id]
    for key, expected_value in expected.items():
        if case[key] != expected_value:
            raise ValueError(f"kernel ingress runtime output closure {key} drift")
    expected_counts = {
        "output_closure_check_count": len(
            RUNTIME_EXECUTION_OUTPUT_CLOSURE_REQUIRED_EVIDENCE_KINDS
        ),
        "output_closure_contract": RUNTIME_EXECUTION_OUTPUT_CLOSURE_CONTRACT,
        "output_closure_policy_id": RUNTIME_EXECUTION_OUTPUT_CLOSURE_POLICY_ID,
        ...
        "source_output_contract_item_count": 1,
        "source_public_output_bundle_item_count": 1,
        "status": "runtime_output_closure_bound",
    }
...
def _assert_bound_digest(actual: str, first_value: object, second_value: object) -> None:
    if actual != first_value or actual != second_value:
        raise ValueError("kernel ingress runtime output closure digest binding drift")


def _assert_digest(value: object) -> None:
    if not isinstance(value, str) or not _SHA256_DIGEST_PATTERN.fullmatch(value):
        raise ValueError("kernel ingress runtime output closure digest drift")
...
def _assert_exact_keys(
    ...
    if set(payload) != expected:
        raise ValueError(f"kernel ingress runtime output closure {context} drift")
...
def _assert_report_is_source_free(report: object) -> None:
    try:
        ...
        raise ValueError("kernel ingress runtime output closure report is not JSON") from exc
    for fragment in (
        SOURCE_TO_INTENT_RESEARCH_KERNEL_INGRESS_RUNTIME_OUTPUT_CLOSURE_INDEX_FORBIDDEN_FRAGMENTS
    ):
        if fragment in text:
            raise ValueError(
                "kernel ingress runtime output closure contains forbidden material"
            )
