...
- Objective Alpha Public Evidence Catalog Extension-Tier Coverage now emits `catalog_required_extension_tiers`, `catalog_missing_extension_tiers`, and `catalog_extension_tier_coverage_status`, and the admission gate requires complete coverage for `governance`, `runtime_proof`, and `frontend_runtime_proof` with no new source, execution, path-resolution, device, plugin, or native-performance surface.
