...
execute_graph
  ...
  -> Runtime Materialized Heterogeneous Storage (opt-in joined storage proof)
...
`Runtime Backend Equivalence Transfer Binding` binds the systolic Runtime
Backend Equivalence report to the verified Transfer Trace Replay Verifier by
metadata digest. It makes the systolic proof slice carry both terminal-output
semantics and transfer-boundary evidence for the same graph. See
[RUNTIME_BACKEND_EQUIVALENCE_TRANSFER_BINDING.md](RUNTIME_BACKEND_EQUIVALENCE_TRANSFER_BINDING.md),
`schemas/runtime_backend_equivalence_transfer_binding_report.v0.schema.json`, and
...
`Runtime Materialized Layout Conversion` is a separate opt-in execution proof.
It preflights the complete conversion plan, reconstructs one bounded padded
`2 x 2` blocked simulator buffer, converts it to a read-only row-major buffer,
verifies exact logical values, and binds the resulting trace to passing mixed
Backend Equivalence. It does not change the historical
`conversion_not_materialized_as_runtime_step` artifacts. See
...
`Runtime Materialized Heterogeneous Storage` is the separate opt-in joined
execution proof. It rebuilds the canonical storage plan before allocation,
preallocates its exact slots, then materializes produced values, blocked-layout
padding, row-major layout staging, and distinct transfer-target staging on the
planned event timeline. Slot generations are reused only after observed
planned releases. The final report binds the storage trace to Reference
Correctness, Backend Equivalence, Materialized Layout Conversion, and
Materialized Transfer without exposing values or storage identity. See
[RUNTIME_MATERIALIZED_HETEROGENEOUS_STORAGE.md](RUNTIME_MATERIALIZED_HETEROGENEOUS_STORAGE.md),
`schemas/runtime_materialized_heterogeneous_storage_report.v0.schema.json`, and
