...
| Evidence Graph Acyclicity Gate | L1 | Data-only DAG gate for the current source-ingestion evidence chain; it proves the First-Slice -> Review -> Approval -> Admission -> Research-Scope digest graph has `cycle_count = 0` while remaining source-free and edge-digest-only. Schema: `schemas/evidence_graph_acyclicity_gate_report.v0.schema.json`; example: `examples/evidence_graph_acyclicity_gate.py`; doc: `docs/EVIDENCE_GRAPH_ACYCLICITY_GATE.md`. |
