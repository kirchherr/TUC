...
| Real Triton First Slice Plan | L0 | Data-only plan for the first possible admitting slice; it binds Admission, Surface Gate Completion, Source Ingestion Quarantine, Admitting Source Ingestion RFC, Bounded Source Buffer API, Source Runtime Smoke, and Kernel Ingress Proof Bundle evidence by digest while keeping `admitted = false` and listing the remaining admission evidence before `direct_source_ingestion` can open. Schema: `schemas/real_triton_first_slice_plan_report.v0.schema.json`; example: `examples/real_triton_first_slice_plan.py`; doc: `docs/REAL_TRITON_FIRST_SLICE_PLAN.md`. |
...
- Real Triton First Slice Plan is documented in
  ...
  `direct_source_ingestion` as the first candidate target surface, but keeps
  admission blocked until RFC, sandbox, fuzz, diagnostics, golden, replay, and
  maintainer-review evidence exists.
