...
Manual placement overrides are policy-gated by
`docs/RUNTIME_OVERRIDE_POLICY.md` and implemented as `RuntimeOverrideSet`. They
may constrain only backend candidates that already passed capability
diagnostics; they cannot create capabilities, change HAC-IR, change
