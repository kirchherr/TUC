...
for fault in 0 1 2 3; do
  ...
  gcc -std=c11 -O2 -Wall -Wextra -Werror -Wconversion -Wformat=2 \
    ...
    -fno-fast-math -frounding-math -fexcess-precision=standard -DTUC_MUL_FAULT="$fault" \
    -fstack-protector-strong -fstack-clash-protection -fcf-protection=full \
    -fno-ident -frandom-seed=tuc-bounded-mul-v0 -static \
    -Wl,--build-id=none,-z,noexecstack,-z,relro,-z,now \
    square-entrypoint.c matrix-entrypoint.c mixed-entrypoint.c gated-entrypoint.c worker.c \
  ...
  gcc -std=c11 -O1 -g1 -Wall -Wextra -Werror -Wconversion -Wformat=2 \
    ...
    -frounding-math -fexcess-precision=standard -DTUC_MUL_FAULT="$fault" \
