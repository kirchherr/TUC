...
SCHEMA = "tuc.bounded_cpu_source_integration.v0"
...
def graph(family, profile):
    ...
    return {
        "schema_version": "source_intent.v0", "name": f"source_{family}_{profile}",
        "tensors": [{"name": name, "shape": list(shape), "dtype": "float32"}
                    for name, shape in tensors],
        "operations": [{"name": output, "family": kind, "inputs": inputs,
                        "outputs": [output], "hints": {},
                        **({"attributes": attributes} if attributes else {})}
                       for _, kind, inputs, output, attributes in operations],
...
def source(family, profile):
    name = f"source_{family}_{profile}"
    if family == "fanout":
        arguments = "a, b, z_positive, a_rows"
        body = ("    p = tl.dot(a, b)\n"
                "    positive = tl.where(p > 0.0, p, 0.0)\n"
                "    rows = tl.sum(p, axis=1)\n"
                "    tl.store(z_positive, positive)\n"
                "    tl.store(a_rows, rows)\n")
    elif family == "truefanin":
        arguments = "a, b, c, d, joined_rows"
        body = ("    left = tl.dot(a, b)\n"
                "    right = tl.dot(c, d)\n"
                "    joined = tl.dot(left, right)\n"
                "    rows = tl.sum(joined, axis=1)\n"
                "    tl.store(joined_rows, rows)\n")
    elif family == "relu_sum":
        arguments = "x, positive_rows"
        body = ("    positive = tl.where(x > 0.0, x, 0.0)\n"
                "    rows = tl.sum(positive, axis=1)\n"
                "    tl.store(positive_rows, rows)\n")
    else:
        raise ValueError("unknown fixed source family")
    if type(profile) is not int or profile not in (0, 1):
        raise ValueError("unknown fixed source profile")
...
def emit():
    ...
    directory = Path(tempfile.mkdtemp(prefix="source-fixtures-", dir=parent))
...
def negative_cases(marker):
    base = source("fanout", 0)
    base_signature = signature("fanout", 0)
    malicious = (
        ("foreign_import", b"import os\n" + base),
        ("eval", b'eval("1 + 1")\n' + base),
        ("file_effect", f"open({str(marker)!r}, 'w').write('forbidden')\n".encode() + base),
        ("network_effect", b'__import__("socket").create_connection(("127.0.0.1", 9))\n' + base),
        ("syntax", base.replace(b"def source_fanout_0(", b"def source_fanout_0((")),
    )
    result = [{"id": name, "source": text, "signature": base_signature,
               "reason": "source_rejected"} for name, text in malicious]
    for name, dimension in (("dimension_limit", 65), ("boolean_dimension", True)):
        altered = signature("fanout", 0)
        altered["tensor_shapes"]["a"][0] = dimension
        result.append({"id": name, "source": base, "signature": altered,
                       "reason": "signature_rejected"})
    changed_name = signature("fanout", 0)
    changed_name["kernel_name"] = "wrong_kernel"
    ...
    nonterminal = (prefix + "def negative_return(x, y, z):\n"
                   ...
                   "    rows = tl.sum(positive, axis=1)\n"
                   "    tl.store(y, positive)\n    tl.store(z, rows)\n").encode()
    result.append({"id": "nonterminal_return", "source": nonterminal, "signature": {
...
def _negative_controls(executable, directory, workspace, snapshots):
    marker = directory / "forbidden-source-effect"
    results = []
    for case in negative_cases(marker):
...
def run():
    ...
    with tempfile.TemporaryDirectory(prefix="tuc-source-cli-consumer-") as temporary:
    ...
    report.update(native_execution_observed=True, source_conversions=conversions,
                  ...
                  control_results=controls, original_files_unchanged=True, workspaces_clean=True)
