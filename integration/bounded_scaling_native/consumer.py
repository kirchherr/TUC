"""Installed fixed Scaling C11 conformance; generation never launches native code."""
...
GRAPHS = ("scalar", "row_scale", "attention", "classifier")
SUPPORT_FILES = ("Dockerfile", "Dockerfile.dockerignore", "build.sh", "operator.sh")
PACKAGE_FILES = (
    "backends/bounded_scaling_dag.py", "backends/bounded_softmax_dag.py",
    "backends/bounded_mul_dag.py",
...
CONTROLS = (
    ...
    "huge_output_count", "nonzero_product_rounding_to_zero", "short_scale", "long_scale",
)
COUNTERS = ("case_runs", "entrypoint_calls", "scalar_checks", "published_outputs",
            "rejected_cases", "sentinel_checks")
SCHEMA = "tuc.bounded_scaling_native_observation.v0"
...
def _read(path, limit=MAX_FILE_BYTES):
    if (path.is_symlink() or any(parent.is_symlink() for parent in path.parents) or
        ...
        raise ValueError("fixed Scaling conformance file rejected")
    with path.open("rb") as stream:
        data = stream.read(limit + 1)
    if len(data) > limit:
        raise ValueError("fixed Scaling conformance file budget rejected")
...
def module(graph):
    if graph not in GRAPHS:
        raise ValueError("fixed Scaling graph rejected")
    mul = {"elementwise_kind": "mul"}
    add = {"elementwise_kind": "add"}
    linear = {"rhs_transposed": True}
    if graph == "scalar":
        shapes = (("x", (5,)), ("scale", (1,)), ("result", (5,)))
        operations = (SourceIntentOperation("scale", "elementwise", ("x", "scale"),
                                             ("result",), attributes=mul),)
    elif graph == "row_scale":
        shapes = (("x", (2, 3)), ("scale", (3,)), ("result", (2, 3)))
        operations = (SourceIntentOperation("scale", "elementwise", ("x", "scale"),
                                             ("result",), attributes=mul),)
    elif graph == "attention":
        shapes = (("q", (2, 4)), ("k", (3, 4)), ("v", (3, 2)), ("scale", (1,)),
                  ("logits", (2, 3)), ("scaled", (2, 3)),
                  ("probabilities", (2, 3)), ("result", (2, 2)))
        operations = (
            SourceIntentOperation("similarity", "matmul", ("q", "k"), ("logits",),
                                  attributes=linear),
            SourceIntentOperation("scale", "elementwise", ("logits", "scale"), ("scaled",),
                                  attributes=mul),
            SourceIntentOperation("normalize", "softmax", ("scaled",), ("probabilities",),
                                  attributes={"axis": 1}),
            SourceIntentOperation("weighted", "matmul", ("probabilities", "v"), ("result",)),
        )
    else:
        shapes = (("x", (2, 3)), ("gain", (3,)), ("offset", (3,)),
                  ("weight", (4, 3)), ("bias", (4,)),
                  ("scaled", (2, 3)), ("calibrated", (2, 3)),
                  ("linear", (2, 4)), ("logits", (2, 4)), ("result", (2, 4)))
        operations = (
            SourceIntentOperation("scale", "elementwise", ("x", "gain"), ("scaled",),
                                  attributes=mul),
            SourceIntentOperation("calibrate", "elementwise", ("scaled", "offset"),
                                  ("calibrated",), attributes=add),
            SourceIntentOperation("project", "matmul", ("calibrated", "weight"),
    ...
    return SourceIntentModule("native_scaling_" + graph, tuple(
...
def fixed_inputs(graph, case):
    if graph not in GRAPHS or type(case) is not int or case not in range(2):
        raise ValueError("fixed Scaling input selector rejected")
    sizes = ({"x": 5, "scale": 1} if graph == "scalar" else
             {"x": 6, "scale": 3} if graph == "row_scale" else
             {"q": 8, "k": 12, "v": 6, "scale": 1} if graph == "attention" else
             {"x": 6, "gain": 3, "offset": 3, "weight": 12, "bias": 4})
    ...
    if graph == "scalar":
        values["x"] = (-0.0, 0.0, 1.0, -2.0, 3.5) if case == 0 else (-1.25, 2.75, -0.0, 0.0, 9.0)
        values["scale"] = (0.5,) if case == 0 else (-0.75,)
    elif graph == "row_scale":
        values["x"] = (-0.0, 1.0, -2.0, 0.0, 3.0, 4.0) if case == 0 else values["x"]
        values["scale"] = (0.5, -1.0, 2.0) if case == 0 else (0.0, -0.75, 1.25)
    elif graph == "attention":
        values["scale"] = (0.5,) if case == 0 else (0.75,)
    return values
...
def reference_outputs(graph, case):
    """Owned scalar FP32 equations; only Softmax compositions use tolerances."""
    ...
    if graph in ("scalar", "row_scale"):
        scale = values["scale"]
        result = [_f32(item * scale[i % len(scale)]) for i, item in enumerate(values["x"])]
    elif graph == "attention":
        logits = linear(values["q"], values["k"], 2, 4, 3)
        probabilities = softmax([_f32(item * values["scale"][0]) for item in logits], 3)
        transposed = [values["v"][row*2+col] for col in range(2) for row in range(3)]
        result = linear(probabilities, transposed, 2, 3, 2)
    else:
        scaled = [_f32(item * values["gain"][i % 3]) for i, item in enumerate(values["x"])]
        calibrated = [_f32(item + values["offset"][i % 3]) for i, item in enumerate(scaled)]
        logits = linear(calibrated, values["weight"], 2, 3, 4)
        result = softmax([_f32(item + values["bias"][i % 4]) for i, item in enumerate(logits)], 4)
...
#include <math.h>
#include <xmmintrin.h>
#ifndef TUC_SCALING_FAULT
#define TUC_SCALING_FAULT 0
#endif
#if TUC_SCALING_FAULT < 0 || TUC_SCALING_FAULT > 3
#error "invalid fixed Scaling conformance control"
#endif
typedef enum tuc_c11_status (*entrypoint_fn)(const struct tuc_c11_input *, size_t,
                                           const struct tuc_c11_output *, size_t);
struct fixture {
  entrypoint_fn run; size_t inputs, elements[7], output_elements, scale_port;
...
static int exercise(size_t graph, size_t input_case, int control, struct counts *counts) {
  ...
  if (control >= 0) {
    ...
    switch (control) {
      ...
      case 21:
      case 28:
        for (size_t i=0; i<fixture->elements[0]; ++i) data[0][i+1U]=1.0F;
        for (size_t i=0; i<fixture->elements[1]; ++i)
          data[1][i+1U] = control==20 ? 2.0F : control==21 ? 0.5F : 0x1p-126F;
        put(&data[0][fixture->elements[0]],
            control==20 ? UINT32_C(0x7f7fffff) : UINT32_C(0x00800000));
        break;
      case 29: --inputs[fixture->scale_port].elements; break;
      case 30: ++inputs[fixture->scale_port].elements; break;
  ...
  if (TUC_SCALING_FAULT==2 && graph==0U && control==0) status=TUC_C11_OK;
  ...
  if (control < 0) {
    if (TUC_SCALING_FAULT==1) output[1] += 0.25F;
    for (size_t i=0; i<fixture->output_elements; ++i) {
      ...
      } else if (!isfinite(actual) ||
                 fabs(actual-reference) > 2e-6 + 2e-5*fabs(reference)) return 1;
    }
    if (graph == 3U) {
    ...
    if (TUC_SCALING_FAULT==3 && graph==0U && control==0) put(&output[1], 0U);
    for (size_t i=0; i<fixture->output_elements; ++i) {
...
static void receipt(const char *status, const char *reason, const struct counts *counts) {
  printf("{\"schema_version\":\"tuc.bounded_scaling_native_observation.v0\","
         ...
         TUC_SCALING_BINDING,status,reason,TUC_SCALING_FAULT,counts->case_runs,counts->entrypoint_calls,
...
int main(int argc, char **argv) {
  ...
  for (size_t graph=0; graph<4U && result==0; ++graph)
    for (int control=0; control<31 && result==0; ++control) {
      if (graph>=2U && (control==20 || control==21 || control==28)) continue;
...
def _worker(compiled, binding):
    ...
    for graph in GRAPHS:
        ...
        scale_port = next(i for i, item in enumerate(inputs)
                          if item.public_name == ("gain" if graph == "classifier" else "scale"))
        table.append("  {" + artifact.entrypoint_symbol + f",{len(inputs)}U," + "{" +
                     ",".join(f"{v}U" for v in sizes + [0] * (7 - len(sizes))) + "}," +
                     f"{output_size}U,{scale_port}U," + "{" + ",".join(cases) + "},{" +
                     ",".join(expected) + "}},")
...
def artifact_files():
    ...
    if re.fullmatch(r"sha256:[0-9a-f]{64}", wheel) is None:
        raise ValueError("fixed Scaling wheel digest rejected")
    ...
    bindings = {"schema_version": "tuc.bounded_scaling_native_bindings.v0",
                "wheel_digest": wheel,
    ...
    if (any(len(text.encode()) > MAX_FILE_BYTES for text in files.values()) or
        ...
        raise ValueError("fixed Scaling context budget rejected")
    return files
...
def _expected(files, fault=0, invalid=False):
    if (type(fault) is not int or not 0 <= fault <= 3 or type(invalid) is not bool or
            (invalid and fault)):
        raise ValueError("fixed Scaling receipt selector rejected")
    counts = ((16, 134, 92, 16, 118, 677), (0, 1, 1, 0, 0, 0),
              (16, 17, 92, 16, 0, 0), (16, 17, 92, 16, 1, 1))[fault]
...
def _receipt_json(text):
    if type(text) is not str or not 1 <= len(text.encode()) <= MAX_RECEIPT_BYTES:
        raise ValueError("fixed Scaling receipt size rejected")
    ...
    for char in text:
        if quoted:
            ...
            depth += 1
            if depth > 4:
                raise ValueError("fixed Scaling receipt depth rejected")
        elif char in "]}":
            depth -= 1
    def reject(_):
        raise ValueError("fixed Scaling receipt number rejected")
    def integer(token):
        if len(token) > 9:
            reject(token)
        return int(token)
    def pairs(items):
        ...
        for key, value in items:
            if key in result:
                raise ValueError("fixed Scaling duplicate receipt key")
            result[key] = value
        return result
...
def _exact(value, expected):
    if (type(value) is not dict or set(value) != set(expected) or
            any(type(value[key]) is not type(expected[key]) or value[key] != expected[key]
                for key in expected)):
        raise ValueError("fixed Scaling receipt rejected")
    return value
...
def report(files=None):
    ...
    return {"schema_version": "tuc.bounded_scaling_native_candidate.v0", "status": "PASS",
            ...
            "controls": list(CONTROLS), "control_exclusions": {
                **{name: [] for name in ("scalar", "row_scale")},
                **{name: [CONTROLS[i] for i in (20, 21, 28)]
                   for name in ("attention", "classifier")}},
            "numerical_tolerance": {"absolute": 2e-6, "relative": 2e-5, "row_mass": 8e-6,
                                    "scalar_and_row_scale": "bitwise_binary32_signed_zero"},
...
def emit_context():
    ...
    if parent.is_symlink() or (parent.exists() and not parent.is_dir()):
        raise ValueError("fixed Scaling context parent rejected")
    parent.mkdir(exist_ok=True)
    directory = Path(tempfile.mkdtemp(prefix="bounded-scaling-native.", dir=parent))
...
def accept(directory):
    ...
    if (path.is_symlink() or any(parent.is_symlink() for parent in path.parents) or
            ...
            re.fullmatch(r"bounded-scaling-native\.[A-Za-z0-9_-]{8,16}", path.name) is None):
        raise ValueError("fixed Scaling evidence path rejected")
    files = artifact_files()
    ...
    for item in path.iterdir():
        if len(actual) > len(files) + len(receipts) + len(images):
            raise ValueError("fixed Scaling evidence count rejected")
        if item.is_symlink() or not item.is_file():
            raise ValueError("fixed Scaling regular evidence required")
        actual.add(item.name)
    if actual - {"record.json"} != set(files) | receipts | images:
        raise ValueError("fixed Scaling evidence coverage rejected")
    for name, text in files.items():
        if _read(path / name) != text:
            raise ValueError("fixed Scaling context changed")
    ...
    for build in ("static", "sanitized"):
        ...
        if re.fullmatch(r"sha256:[0-9a-f]{64}", image) is None:
            raise ValueError("fixed Scaling image identity rejected")
        image_ids[build] = image
    return {**report(files), "schema_version": "tuc.bounded_scaling_native_record.v0",
            "native_execution_observed": True, "observation_scope": "four_fixed_scaling_cpu_graphs",
...
def main(argv=None):
    ...
    try:
        ...
        print("fixed Scaling conformance rejected", file=sys.stderr)
        return 1
