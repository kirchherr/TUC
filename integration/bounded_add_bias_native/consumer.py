"""Installed fixed Add/Bias C11 conformance; generation never launches native code."""
...
GRAPHS = ("vector", "matrix", "bias", "pipeline")
SUPPORT_FILES = ("Dockerfile", "Dockerfile.dockerignore", "build.sh", "operator.sh")
PACKAGE_FILES = (
    "backends/bounded_add_dag.py", "backends/bounded_c11_codegen.py",
...
CONTROLS = (
    ...
    "subnormal_input", "overflow_addition", "subnormal_addition", "rounding_mode",
...
SCHEMA = "tuc.bounded_add_bias_native_observation.v0"
...
def _read(path, limit=MAX_FILE_BYTES):
    if (path.is_symlink() or any(parent.is_symlink() for parent in path.parents) or
        ...
        raise ValueError("fixed Add conformance file rejected")
    with path.open("rb") as stream:
        data = stream.read(limit + 1)
    if len(data) > limit:
        raise ValueError("fixed Add conformance file budget rejected")
...
def module(graph):
    if graph not in GRAPHS:
        raise ValueError("fixed Add graph rejected")
    if graph != "pipeline":
        lhs = (5,) if graph == "vector" else (2, 3)
        rhs = (3,) if graph == "bias" else lhs
        return SourceIntentModule("native_" + graph, tuple(
            SourceIntentTensor(name, shape) for name, shape in (("left", lhs), ("right", rhs),
                                                              ("result", lhs))), (
            SourceIntentOperation("sum", "elementwise", ("left", "right"), ("result",),
                                  attributes={"elementwise_kind": "add"}),
        ), returns=(SourceIntentReturn("sum", "result"),))
    return SourceIntentModule("native_pipeline", tuple(SourceIntentTensor(name, shape)
        for name, shape in (("x", (2, 3)), ("weights", (3, 4)), ("bias", (4,)),
            ("residual", (2, 4)), ("p", (2, 4)), ("biased", (2, 4)), ("active", (2, 4)),
            ("joined", (2, 4)), ("rows", (2,)))), (
...
def fixed_inputs(graph, case):
    if graph not in GRAPHS or type(case) is not int or case not in range(3):
        raise ValueError("fixed Add input selector rejected")
    sizes = ({"left": 5, "right": 5} if graph == "vector" else
             {"left": 6, "right": 6} if graph == "matrix" else
             {"left": 6, "right": 3} if graph == "bias" else
             {"x": 6, "weights": 12, "bias": 4, "residual": 8})
    values = {}
    for port, (name, size) in enumerate(sizes.items()):
        if case == 2:
            values[name] = tuple(-0.0 if (i + port) % 3 else 0.0 for i in range(size))
        else:
...
def reference_outputs(graph, case):
    ...
    if graph != "pipeline":
        right = values["right"]
        return {"sum": tuple(_f32(a + right[i % len(right)])
                             for i, a in enumerate(values["left"]))}
    rows = []
    for row in range(2):
        total = 0.0
        for column in range(4):
            product = 0.0
            for k in range(3):
                product = _f32(product + _f32(values["x"][row * 3 + k] *
                                              values["weights"][k * 4 + column]))
            biased = _f32(product + values["bias"][column])
            active = 0.0 if biased < 0.0 else biased
            joined = _f32(active + values["residual"][row * 4 + column])
            total = _f32(total + joined)
        rows.append(total)
    return {"scores": tuple(rows)}
...
#include <fenv.h>
#include <xmmintrin.h>
#ifndef TUC_ADD_FAULT
#define TUC_ADD_FAULT 0
#endif
#if TUC_ADD_FAULT < 0 || TUC_ADD_FAULT > 3
#error "invalid fixed Add conformance control"
#endif
typedef enum tuc_c11_status (*entrypoint_fn)(const struct tuc_c11_input *, size_t,
                                           const struct tuc_c11_output *, size_t);
struct fixture {
  entrypoint_fn run; size_t inputs, elements[4], output_elements;
  const uint32_t *data[3][4]; const uint32_t *expected[3];
};
struct counts {
...
static int exercise(size_t graph, size_t input_case, int control, struct counts *counts) {
  ...
  float data[4][14], original[4][14], output[8];
  struct tuc_c11_input inputs[4]; struct tuc_c11_output outputs[1];
  for (size_t p=0; p<4U; ++p)
    for (size_t i=0; i<14U; ++i) put(&data[p][i], UINT32_C(0x42f68000));
  for (size_t i=0; i<8U; ++i) put(&output[i], UINT32_C(0x42f68000));
  ...
  if (control >= 0) {
    expected = control >= 17 && control <= 21 ? TUC_C11_NUMERIC :
    ...
    switch (control) {
      ...
      case 21:
        ...
        if (graph == 3U) {
          /* Zero matmul, then normal bias; the residual Add must reject. */
          for (size_t i=0; i<fixture->elements[2]; ++i)
            put(&data[2][i+1U], control==20 ? UINT32_C(0x7f7fffff) : UINT32_C(0x00800001));
          for (size_t i=0; i<fixture->elements[3]; ++i)
            put(&data[3][i+1U], control==20 ? UINT32_C(0x7f7fffff) : UINT32_C(0x80800000));
        } else {
          for (size_t i=0; i<fixture->elements[0]; ++i)
            put(&data[0][i+1U], control==20 ? UINT32_C(0x7f7fffff) : UINT32_C(0x00800001));
          for (size_t i=0; i<fixture->elements[1]; ++i)
            put(&data[1][i+1U], control==20 ? UINT32_C(0x7f7fffff) : UINT32_C(0x80800000));
        }
        break;
  ...
  if (control < 0) {
    ...
    if (TUC_ADD_FAULT==3 && graph==0U && control==0) put(&output[1], 0U);
    for (size_t i=0; i<fixture->output_elements; ++i) {
...
static void receipt(const char *status, const char *reason, const struct counts *counts) {
  printf("{\"schema_version\":\"tuc.bounded_add_bias_native_observation.v0\","
         ...
         TUC_ADD_BINDING,status,reason,TUC_ADD_FAULT,counts->case_runs,counts->entrypoint_calls,
...
def _worker(compiled, binding):
    lines = [_WORKER, '#define TUC_ADD_BINDING "' + binding + '"']
    table = []
    for graph in GRAPHS:
        compilation, artifact = compiled[graph]
        inputs = compilation.input_bindings
        if len(inputs) > 4 or len(compilation.output_bindings) != 1:
            raise ValueError("fixed Add graph arity changed")
        sizes = [len(fixed_inputs(graph, 0)[item.public_name]) for item in inputs]
        if max(sizes) > 12:
            raise ValueError("fixed Add input extent changed")
        output = compilation.output_bindings[0].public_name
        output_size = len(reference_outputs(graph, 0)[output])
        if not 1 <= output_size <= 6:
            raise ValueError("fixed Add output extent changed")
        cases, expected = [], []
        for case in range(3):
            names = []
            for port, item in enumerate(inputs):
                ...
                names.append(name)
            cases.append("{" + ",".join(names + ["NULL"] * (4 - len(names))) + "}")
        ...
        table.append("  {" + artifact.entrypoint_symbol + f",{len(inputs)}U," + "{" +
                     ",".join(f"{v}U" for v in sizes + [0] * (4 - len(sizes))) + "}," +
...
def artifact_files():
    ...
    if re.fullmatch(r"sha256:[0-9a-f]{64}", wheel) is None:
        raise ValueError("fixed Add wheel digest rejected")
    ...
    bindings = {"schema_version": "tuc.bounded_add_bias_native_bindings.v0",
                "wheel_digest": wheel,
    ...
    if (any(len(text.encode()) > MAX_FILE_BYTES for text in files.values()) or
        ...
        raise ValueError("fixed Add context budget rejected")
    return files
...
def _expected(files, fault=0, invalid=False):
    if (type(fault) is not int or not 0 <= fault <= 3 or type(invalid) is not bool or
            (invalid and fault)):
        raise ValueError("fixed Add receipt selector rejected")
    counts = ((24, 136, 114, 24, 112, 532), (0, 1, 1, 0, 0, 0),
              (24, 25, 114, 24, 0, 0), (24, 25, 114, 24, 1, 1))[fault]
...
def _receipt_json(text):
    if type(text) is not str or not 1 <= len(text.encode()) <= MAX_RECEIPT_BYTES:
        raise ValueError("fixed Add receipt size rejected")
    ...
    for char in text:
        if quoted:
            ...
            depth += 1
            if depth > 4:
                raise ValueError("fixed Add receipt depth rejected")
        elif char in "]}":
            depth -= 1
    def reject(_):
        raise ValueError("fixed Add receipt number rejected")
    def integer(token):
        if len(token) > 9:
            reject(token)
        return int(token)
    def pairs(items):
        ...
        for key, value in items:
            if key in result:
                raise ValueError("fixed Add duplicate receipt key")
            result[key] = value
        return result
...
def _exact(value, expected):
    if (type(value) is not dict or set(value) != set(expected) or
            any(type(value[key]) is not type(expected[key]) or value[key] != expected[key]
                for key in expected)):
        raise ValueError("fixed Add receipt rejected")
    return value
...
def report(files=None):
    ...
    return {"schema_version": "tuc.bounded_add_bias_native_candidate.v0", "status": "PASS",
            ...
            "controls": list(CONTROLS), "expected_baseline": _expected(files),
...
def emit_context():
    ...
    if parent.is_symlink() or (parent.exists() and not parent.is_dir()):
        raise ValueError("fixed Add context parent rejected")
    parent.mkdir(exist_ok=True)
    directory = Path(tempfile.mkdtemp(prefix="bounded-add-bias-native.", dir=parent))
...
def accept(directory):
    ...
    if (path.is_symlink() or any(parent.is_symlink() for parent in path.parents) or
            ...
            re.fullmatch(r"bounded-add-bias-native\.[A-Za-z0-9_-]{8,16}", path.name) is None):
        raise ValueError("fixed Add evidence path rejected")
    files = artifact_files()
    ...
    for item in path.iterdir():
        if len(actual) > len(files) + len(receipts) + len(images):
            raise ValueError("fixed Add evidence count rejected")
        if item.is_symlink() or not item.is_file():
            raise ValueError("fixed Add regular evidence required")
        actual.add(item.name)
    if actual - {"record.json"} != set(files) | receipts | images:
        raise ValueError("fixed Add evidence coverage rejected")
    for name, text in files.items():
        if _read(path / name) != text:
            raise ValueError("fixed Add context changed")
    ...
    for build in ("static", "sanitized"):
        ...
        if re.fullmatch(r"sha256:[0-9a-f]{64}", image) is None:
            raise ValueError("fixed Add image identity rejected")
        image_ids[build] = image
    return {**report(files), "schema_version": "tuc.bounded_add_bias_native_record.v0",
            "native_execution_observed": True, "observation_scope": "four_fixed_cpu_graphs",
...
def main(argv=None):
    ...
    try:
        ...
        print("fixed Add conformance rejected", file=sys.stderr)
        return 1
