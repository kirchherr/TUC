"""Installed fixed Linear C11 conformance; generation never launches native code."""
...
GRAPHS = ("nonsquare", "singleton", "mixed", "mlp")
SUPPORT_FILES = ("Dockerfile", "Dockerfile.dockerignore", "build.sh", "operator.sh")
PACKAGE_FILES = (
    "backends/bounded_linear_dag.py", "backends/bounded_add_dag.py",
...
SCHEMA = "tuc.bounded_linear_native_observation.v0"
...
def _read(path, limit=MAX_FILE_BYTES):
    if (path.is_symlink() or any(parent.is_symlink() for parent in path.parents) or
        ...
        raise ValueError("fixed Linear conformance file rejected")
    with path.open("rb") as stream:
        data = stream.read(limit + 1)
    if len(data) > limit:
        raise ValueError("fixed Linear conformance file budget rejected")
...
def module(graph):
    if graph not in GRAPHS:
        raise ValueError("fixed Linear graph rejected")
    linear = {"rhs_transposed": True}
    add = {"elementwise_kind": "add"}
    if graph in ("nonsquare", "singleton"):
        m, k, n = (2, 3, 4) if graph == "nonsquare" else (1, 1, 1)
        shapes = (("x", (m, k)), ("weight", (n, k)), ("result", (m, n)))
        operations = (SourceIntentOperation("linear", "matmul", ("x", "weight"), ("result",),
                                             attributes=linear),)
    elif graph == "mixed":
        shapes = (("x", (3, 2)), ("weight", (4, 2)), ("other", (4, 2)),
                  ("linear", (3, 4)), ("product", (3, 2)), ("result", (3,)))
        operations = (
            SourceIntentOperation("linear", "matmul", ("x", "weight"), ("linear",),
                                  attributes=linear),
            SourceIntentOperation("ordinary", "matmul", ("linear", "other"), ("product",)),
            SourceIntentOperation("sum", "reduction", ("product",), ("result",),
                                  attributes={"axis": 1}),
        )
    else:
        shapes = (("x", (2, 3)), ("w1", (4, 3)), ("b1", (4,)), ("w2", (2, 4)),
                  ("b2", (2,)), ("skip", (2, 2)), ("p1", (2, 4)), ("biased1", (2, 4)),
                  ("relu", (2, 4)), ("p2", (2, 2)), ("biased2", (2, 2)),
                  ("residual", (2, 2)), ("result", (2,)))
        operations = (
            ...
            SourceIntentOperation("linear2", "matmul", ("relu", "w2"), ("p2",), attributes=linear),
    ...
    return SourceIntentModule("native_linear_" + graph, tuple(
...
def fixed_inputs(graph, case):
    if graph not in GRAPHS or type(case) is not int or case not in range(2):
        raise ValueError("fixed Linear input selector rejected")
    sizes = ({"x": 6, "weight": 12} if graph == "nonsquare" else
             {"x": 1, "weight": 1} if graph == "singleton" else
             {"x": 6, "weight": 8, "other": 8} if graph == "mixed" else
             {"x": 6, "w1": 12, "b1": 4, "w2": 8, "b2": 2, "skip": 4})
    divisor = 4.0 if case == 0 else 7.0
    return {name: tuple(_f32(((i * (port + 2) + port) % 13 - 6) / divisor)
...
def reference_outputs(graph, case):
    ...
    if graph in ("nonsquare", "singleton"):
        m, k, n = (2, 3, 4) if graph == "nonsquare" else (1, 1, 1)
        return {"scores": tuple(linear(values["x"], values["weight"], m, k, n))}
    if graph == "mixed":
        ...
        for row in range(3):
            ...
            for column in range(2):
                ...
                for inner in range(4):
                    total = _f32(total + _f32(stage[row * 4 + inner] *
        ...
        second = linear(active, values["w2"], 2, 4, 2)
...
#include <fenv.h>
#include <xmmintrin.h>
#ifndef TUC_LINEAR_FAULT
#define TUC_LINEAR_FAULT 0
#endif
#if TUC_LINEAR_FAULT < 0 || TUC_LINEAR_FAULT > 3
#error "invalid fixed Linear conformance control"
#endif
typedef enum tuc_c11_status (*entrypoint_fn)(const struct tuc_c11_input *, size_t,
                                           const struct tuc_c11_output *, size_t);
struct fixture {
  entrypoint_fn run; size_t inputs, elements[6], output_elements;
  const uint32_t *data[2][6]; const uint32_t *expected[2];
};
struct counts {
...
static int exercise(size_t graph, size_t input_case, int control, struct counts *counts) {
  ...
  float data[6][14], original[6][14], output[10];
  struct tuc_c11_input inputs[6]; struct tuc_c11_output outputs[1];
  ...
  if (control >= 0) {
    ...
    switch (control) {
      ...
      case 28:
        ...
        put(&data[0][1], control==20 ? UINT32_C(0x7f7fffff) :
                          control==21 ? UINT32_C(0x00800000) : UINT32_C(0x0d800000));
        put(&data[1][1], control==20 ? UINT32_C(0x7f7fffff) :
  ...
  if (TUC_LINEAR_FAULT==2 && graph==0U && control==0) status=TUC_C11_OK;
  ...
  if (control < 0) {
    if (TUC_LINEAR_FAULT==1) put(&output[1], bits(&output[1]) ^ UINT32_C(1));
    ...
    if (TUC_LINEAR_FAULT==3 && graph==0U && control==0) put(&output[1], 0U);
    for (size_t i=0; i<fixture->output_elements; ++i) {
...
static void receipt(const char *status, const char *reason, const struct counts *counts) {
  printf("{\"schema_version\":\"tuc.bounded_linear_native_observation.v0\","
         ...
         TUC_LINEAR_BINDING,status,reason,TUC_LINEAR_FAULT,counts->case_runs,counts->entrypoint_calls,
    ...
    for graph in GRAPHS:
        ...
        for case in range(2):
            ...
            for port, item in enumerate(inputs):
                ...
                names.append(name)
            cases.append("{" + ",".join(names + ["NULL"] * (6 - len(names))) + "}")
        ...
        table.append("  {" + artifact.entrypoint_symbol + f",{len(inputs)}U," + "{" +
                     ",".join(f"{v}U" for v in sizes + [0] * (6 - len(sizes))) + "}," +
...
def artifact_files():
    ...
    if re.fullmatch(r"sha256:[0-9a-f]{64}", wheel) is None:
        raise ValueError("fixed Linear wheel digest rejected")
    ...
    bindings = {"schema_version": "tuc.bounded_linear_native_bindings.v0",
                "wheel_digest": wheel,
    ...
    if (any(len(text.encode()) > MAX_FILE_BYTES for text in files.values()) or
        ...
        raise ValueError("fixed Linear context budget rejected")
    return files
...
def _expected(files, fault=0, invalid=False):
    if (type(fault) is not int or not 0 <= fault <= 3 or type(invalid) is not bool or
            (invalid and fault)):
        raise ValueError("fixed Linear receipt selector rejected")
    counts = ((16, 132, 56, 16, 116, 406), (0, 1, 1, 0, 0, 0),
              (16, 17, 56, 16, 0, 0), (16, 17, 56, 16, 1, 1))[fault]
...
def _receipt_json(text):
    if type(text) is not str or not 1 <= len(text.encode()) <= MAX_RECEIPT_BYTES:
        raise ValueError("fixed Linear receipt size rejected")
    ...
    for char in text:
        if quoted:
            ...
            depth += 1
            if depth > 4:
                raise ValueError("fixed Linear receipt depth rejected")
        elif char in "]}":
            depth -= 1
    def reject(_):
        raise ValueError("fixed Linear receipt number rejected")
    def integer(token):
        if len(token) > 9:
            reject(token)
        return int(token)
    def pairs(items):
        ...
        for key, value in items:
            if key in result:
                raise ValueError("fixed Linear duplicate receipt key")
            result[key] = value
        return result
...
def _exact(value, expected):
    if (type(value) is not dict or set(value) != set(expected) or
            any(type(value[key]) is not type(expected[key]) or value[key] != expected[key]
                for key in expected)):
        raise ValueError("fixed Linear receipt rejected")
    return value
...
def report(files=None):
    ...
    return {"schema_version": "tuc.bounded_linear_native_candidate.v0", "status": "PASS",
...
def emit_context():
    ...
    if parent.is_symlink() or (parent.exists() and not parent.is_dir()):
        raise ValueError("fixed Linear context parent rejected")
    parent.mkdir(exist_ok=True)
    directory = Path(tempfile.mkdtemp(prefix="bounded-linear-native.", dir=parent))
...
def accept(directory):
    ...
    if (path.is_symlink() or any(parent.is_symlink() for parent in path.parents) or
            ...
            re.fullmatch(r"bounded-linear-native\.[A-Za-z0-9_-]{8,16}", path.name) is None):
        raise ValueError("fixed Linear evidence path rejected")
    files = artifact_files()
    ...
    for item in path.iterdir():
        if len(actual) > len(files) + len(receipts) + len(images):
            raise ValueError("fixed Linear evidence count rejected")
        if item.is_symlink() or not item.is_file():
            raise ValueError("fixed Linear regular evidence required")
        actual.add(item.name)
    if actual - {"record.json"} != set(files) | receipts | images:
        raise ValueError("fixed Linear evidence coverage rejected")
    for name, text in files.items():
        if _read(path / name) != text:
            raise ValueError("fixed Linear context changed")
    ...
    for build in ("static", "sanitized"):
        ...
        if re.fullmatch(r"sha256:[0-9a-f]{64}", image) is None:
            raise ValueError("fixed Linear image identity rejected")
        image_ids[build] = image
    return {**report(files), "schema_version": "tuc.bounded_linear_native_record.v0",
            "native_execution_observed": True, "observation_scope": "four_fixed_linear_cpu_graphs",
...
def main(argv=None):
    ...
    try:
        ...
        print("fixed Linear conformance rejected", file=sys.stderr)
        return 1
