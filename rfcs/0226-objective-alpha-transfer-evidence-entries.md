...
Add Runtime Transfer Trace Replay Verifier and Runtime Backend Equivalence
Transfer Binding as fixed digest-only entries in the Objective Alpha Public
Proof Bundle.

## Motivation

Runtime Evidence Gate now requires the systolic backend-equivalence proof slice
to carry both verified transfer trace replay and backend-equivalence/transfer
binding evidence. Reviewers should be able to see those proof boundaries from
the public Objective Alpha entry point without unpacking the gate output.
...
Runtime Backend Equivalence, Runtime Transfer Trace Replay Verifier, and Runtime
Backend Equivalence Transfer Binding remain gate-bound while also becoming
reviewer-visible public bundle entries.