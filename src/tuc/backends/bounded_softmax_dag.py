"""CPU-only row Softmax, independent of frozen prior DAG emitters.

Only validated rank-two binary32 graphs enter this inert source boundary.
...
from tuc.backends.bounded_mul_dag import _mul_operation
...
SCHEMA_VERSION = "tuc.bounded_softmax_dag_artifacts.v0"
...
def _softmax_operation(op: ComputeOperation, index: int) -> tuple[KernelSpec, int]:
    ...
    for key in legacy._BOOLEAN_HINTS.intersection(semantics):
        if type(op.attributes[key]) is not bool:
            raise ValueError("bounded Softmax hint rejected")
    ...
    if "max_error_budget" in semantics:
        ...
        if type(budget) not in (int, float) or cast(float, budget) < 0:
            raise ValueError("bounded Softmax error hint rejected")
    ...
    if (semantics != {"axis"} or type(axis) is not int or axis != 1 or
            len(op.inputs) != 1 or type(layout) not in (str, LayoutKind) or
            layout != LayoutKind.ROW_MAJOR or type(tile) is not tuple or tile):
        raise ValueError("bounded Softmax semantics or layout rejected")
    shape = op.inputs[0].shape
    out = op.outputs[0].shape
    if (len(shape) != 2 or out != shape or
            op.outputs[0].name == op.inputs[0].name):
        raise ValueError("bounded Softmax shapes rejected")
    return KernelSpec(index, "softmax_axis1", (shape,), out), 5 * prod(out)
...
def _checked_graph(module: IRModule) -> tuple[tuple[KernelSpec, ...], tuple[int, ...]]:
    ...
    if (module.stage is not IRStage.HAC_IR or module.target is not None or
        ...
        raise ValueError("bounded Softmax HAC-IR rejected")
    graph = module.graph
    ...
    if (type(graph.operations) is not tuple or not 1 <= len(graph.operations) <= 8 or
        ...
        raise ValueError("bounded Softmax graph rejected")
    ...
    for index, op in enumerate(graph.operations):
        ...
        if (type(op.kind) is not OperationKind or type(op.inputs) is not tuple or
                ...
                op.name in names):
            raise ValueError("bounded Softmax operation rejected")
        names.add(op.name)
        ...
        for tensor in (*op.inputs, *op.outputs):
            ...
            if previous is not None and (previous.shape != tensor.shape or
                ...
                raise ValueError("bounded Softmax tensor identity rejected")
    ...
    if (len(tensors) > legacy.MAX_DAG_TENSORS or
            ...
            not any(spec.kind == "softmax_axis1" for spec in specs)):
        raise ValueError("bounded Softmax graph budget or operation scope rejected")
    _safe_metadata((module.metadata, graph.metadata,
...
def _primitives(specs: tuple[KernelSpec, ...]) -> tuple[str, str, str]:
    header = ["#ifndef TUC_BOUNDED_SOFTMAX_GENERATED_H", "#define TUC_BOUNDED_SOFTMAX_GENERATED_H",
    ...
    source = ['#include "generated.h"', "#include <stddef.h>", "#include <float.h>",
              "#include <math.h>",
              "#ifdef __FAST_MATH__", '#error "bounded Softmax forbids fast math"', "#endif",
              '_Static_assert(sizeof(float) == 4 && FLT_RADIX == 2 && FLT_MANT_DIG == 24,',
              '               "bounded Softmax requires binary32");',
              '_Static_assert(FLT_MAX_EXP == 128 && FLT_MIN_EXP == -125 && FLT_EVAL_METHOD == 0,',
              '               "bounded Softmax requires binary32 evaluation");',
    ...
    cuda = '#error "bounded Softmax DAG is CPU-only; no CUDA source is emitted"\n'
    return "\n".join(header), "\n".join(source), cuda
...
def _schedule(module: IRModule, plan: ResidencyPlan, specs: tuple[KernelSpec, ...],
    ...
    lines = ["#ifndef TUC_BOUNDED_SOFTMAX_SCHEDULE_H", "#define TUC_BOUNDED_SOFTMAX_SCHEDULE_H",
             "/* CPU-only static data; no allocation or dispatch authority. */",
             "enum tuc_softmax_op_kind { TUC_SOFTMAX_MATMUL=0, TUC_SOFTMAX_RELU=1,",
             "  TUC_SOFTMAX_SUM_AXIS1=2,",
             "  TUC_SOFTMAX_EQUAL=3, TUC_SOFTMAX_ROW_BIAS=4, TUC_SOFTMAX_MATMUL_RHS_TRANSPOSED=5,",
             "  TUC_SOFTMAX_EQUAL_PRODUCT=6, TUC_SOFTMAX_AXIS1=7 };",
             "struct tuc_softmax_buffer { unsigned int tensor, bytes; };",
             "struct tuc_softmax_operation { unsigned int kind, input0, input1, output; };",
             "struct tuc_softmax_event { unsigned int kind, operation, input0, input1, output; };",
             f"#define TUC_SOFTMAX_BUFFER_COUNT {len(plan.buffers)}U",
             f"#define TUC_SOFTMAX_OPERATION_COUNT {len(specs)}U",
             f"#define TUC_SOFTMAX_EVENT_COUNT {len(plan.steps)}U",
             "static const struct tuc_softmax_buffer tuc_softmax_buffers[] = {"]
    lines += [f"  {{{tensors[b.tensor]}U, {b.bytes}U}}," for b in plan.buffers]
    lines += ["};", "static const struct tuc_softmax_operation tuc_softmax_operations[] = {"]
    ...
    lines += ["};", "static const struct tuc_softmax_event tuc_softmax_events[] = {"]
...
def lower_bounded_softmax_dag(hac_ir: IRModule, partition: PartitionPlan,
                          targets: dict[str, DAGTarget]) -> BoundedDAGArtifacts:
    """Validate and emit one CPU-only graph containing explicit hardware-neutral Softmax."""
    ...
    if (len(plan.buffers) > legacy.MAX_DAG_BUFFERS or len(plan.steps) > legacy.MAX_DAG_EVENTS or
        ...
        raise ValueError("bounded Softmax residency budget or CPU scope rejected")
    ...
    if len(hac_dump.encode("utf-8")) > legacy.MAX_DAG_METADATA_BYTES:
        raise ValueError("bounded Softmax HAC dump budget rejected")
...
def validate_bounded_softmax_dag_artifacts(artifacts: BoundedDAGArtifacts, hac_ir: IRModule,
    ...
    legacy._check_artifact_budget(artifacts)
    expected = lower_bounded_softmax_dag(hac_ir, partition, targets)
    for item in fields(BoundedDAGArtifacts):
        if getattr(artifacts, item.name) != getattr(expected, item.name):
            raise ValueError("bounded Softmax artifact differs from source")


__all__ = ["lower_bounded_softmax_dag", "validate_bounded_softmax_dag_artifacts"]
