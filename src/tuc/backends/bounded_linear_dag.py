"""CPU-only Linear extension, preserving frozen DAG/Add emission boundaries.

Explicit rhs_transposed=True means X[M,K] times stored W[N,K]. Only validated
typed data enters code generation; no transpose allocation or execution occurs.
"""
...
SCHEMA_VERSION = "tuc.bounded_linear_dag_artifacts.v0"
_KINDS = {"matmul": 0, "relu": 1, "sum_axis1": 2, "add": 3, "add_row_bias": 4,
          "matmul_rhs_transposed": 5}
_EVENTS = {"bind_input": 0, "copy": 1, "execute": 2, "publish_output": 3}


def _linear_operation(op: ComputeOperation, index: int) -> tuple[KernelSpec, int]:
    ...
    for key in legacy._BOOLEAN_HINTS.intersection(semantics):
        if type(op.attributes[key]) is not bool:
            raise ValueError("bounded Linear hint rejected")
    ...
    if "max_error_budget" in semantics:
        ...
        if type(budget) not in (int, float) or cast(float, budget) < 0:
            raise ValueError("bounded Linear error hint rejected")
        semantics.remove("max_error_budget")
    layout = op.attributes.get("tuc.layout")
    tile = op.attributes.get("tuc.layout_tile_shape")
    if (semantics != {"rhs_transposed"} or op.attributes["rhs_transposed"] is not True or
            len(op.inputs) != 2 or type(layout) not in (str, LayoutKind) or
            layout != LayoutKind.ROW_MAJOR or type(tile) is not tuple or tile):
        raise ValueError("bounded Linear semantics or layout rejected")
    lhs, rhs = (tensor.shape for tensor in op.inputs)
    out = op.outputs[0].shape
    if (len(lhs) != 2 or len(rhs) != 2 or lhs[1] != rhs[1] or
            out != (lhs[0], rhs[0])):
        raise ValueError("bounded Linear shapes rejected")
    return KernelSpec(index, "matmul_rhs_transposed", (lhs, rhs), out), 2 * prod(out) * lhs[1]
...
def _checked_graph(module: IRModule) -> tuple[tuple[KernelSpec, ...], tuple[int, ...]]:
    ...
    if (module.stage is not IRStage.HAC_IR or module.target is not None or
        ...
        raise ValueError("bounded Linear HAC-IR rejected")
    graph = module.graph
    ...
    if (type(graph.operations) is not tuple or not 1 <= len(graph.operations) <= 8 or
        ...
        raise ValueError("bounded Linear graph rejected")
    ...
    for index, op in enumerate(graph.operations):
        ...
        if (type(op.kind) is not OperationKind or type(op.inputs) is not tuple or
                ...
                op.name in names):
            raise ValueError("bounded Linear operation rejected")
        names.add(op.name)
        ...
        for tensor in (*op.inputs, *op.outputs):
            ...
            if previous is not None and (previous.shape != tensor.shape or
                ...
                raise ValueError("bounded Linear tensor identity rejected")
            tensors[tensor.name] = tensor
        if op.kind is OperationKind.MATMUL and "rhs_transposed" in op.attributes:
    ...
    if (len(tensors) > legacy.MAX_DAG_TENSORS or
            ...
            not any(spec.kind == "matmul_rhs_transposed" for spec in specs)):
        raise ValueError("bounded Linear graph budget or operation scope rejected")
    _safe_metadata((module.metadata, graph.metadata,
...
def _primitives(specs: tuple[KernelSpec, ...]) -> tuple[str, str, str]:
    header = ["#ifndef TUC_BOUNDED_LINEAR_GENERATED_H", "#define TUC_BOUNDED_LINEAR_GENERATED_H",
    ...
    source = ['#include "generated.h"', "#include <stddef.h>", "#include <float.h>",
              "#ifdef __FAST_MATH__", '#error "bounded Linear forbids fast math"', "#endif",
              '_Static_assert(sizeof(float) == 4 && FLT_RADIX == 2 && FLT_MANT_DIG == 24,',
              '               "bounded Linear requires binary32");',
              '_Static_assert(FLT_MAX_EXP == 128 && FLT_MIN_EXP == -125 && FLT_EVAL_METHOD == 0,',
              '               "bounded Linear requires binary32 evaluation");',
    ...
    cuda = '#error "bounded Linear DAG is CPU-only; no CUDA source is emitted"\n'
    return "\n".join(header), "\n".join(source), cuda
...
def _schedule(module: IRModule, plan: ResidencyPlan, specs: tuple[KernelSpec, ...],
    ...
    lines = ["#ifndef TUC_BOUNDED_LINEAR_SCHEDULE_H", "#define TUC_BOUNDED_LINEAR_SCHEDULE_H",
             "/* CPU-only static data; no allocation or dispatch authority. */",
             "enum tuc_linear_op_kind { TUC_LINEAR_MATMUL=0, TUC_LINEAR_RELU=1,",
             "  TUC_LINEAR_SUM_AXIS1=2,",
             "  TUC_LINEAR_EQUAL=3, TUC_LINEAR_ROW_BIAS=4, TUC_LINEAR_MATMUL_RHS_TRANSPOSED=5 };",
             "struct tuc_linear_buffer { unsigned int tensor, bytes; };",
             "struct tuc_linear_operation { unsigned int kind, input0, input1, output; };",
             "struct tuc_linear_event { unsigned int kind, operation, input0, input1, output; };",
             f"#define TUC_LINEAR_BUFFER_COUNT {len(plan.buffers)}U",
             f"#define TUC_LINEAR_OPERATION_COUNT {len(specs)}U",
             f"#define TUC_LINEAR_EVENT_COUNT {len(plan.steps)}U",
             "static const struct tuc_linear_buffer tuc_linear_buffers[] = {"]
    lines += [f"  {{{tensors[b.tensor]}U, {b.bytes}U}}," for b in plan.buffers]
    lines += ["};", "static const struct tuc_linear_operation tuc_linear_operations[] = {"]
    ...
    lines += ["};", "static const struct tuc_linear_event tuc_linear_events[] = {"]
...
def lower_bounded_linear_dag(hac_ir: IRModule, partition: PartitionPlan,
                          targets: dict[str, DAGTarget]) -> BoundedDAGArtifacts:
    """Validate and emit one CPU-only graph containing explicit hardware-neutral Linear."""
    ...
    if (len(plan.buffers) > legacy.MAX_DAG_BUFFERS or len(plan.steps) > legacy.MAX_DAG_EVENTS or
        ...
        raise ValueError("bounded Linear residency budget or CPU scope rejected")
    ...
    if len(hac_dump.encode("utf-8")) > legacy.MAX_DAG_METADATA_BYTES:
        raise ValueError("bounded Linear HAC dump budget rejected")
...
def validate_bounded_linear_dag_artifacts(artifacts: BoundedDAGArtifacts, hac_ir: IRModule,
    ...
    legacy._check_artifact_budget(artifacts)
    expected = lower_bounded_linear_dag(hac_ir, partition, targets)
    for item in fields(BoundedDAGArtifacts):
        if getattr(artifacts, item.name) != getattr(expected, item.name):
            raise ValueError("bounded Linear artifact differs from source")


__all__ = ["lower_bounded_linear_dag", "validate_bounded_linear_dag_artifacts"]
