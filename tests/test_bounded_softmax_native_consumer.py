"""Independent fixed Softmax oracle and receipt-boundary tests; no native execution."""
...
from integration.bounded_softmax_native import consumer
...
def numpy_reference(graph, case):
    ...
    if graph == "softmax":
        result = softmax(data["x"].reshape(2, 3))
    elif graph == "classifier":
        logits = np.add(linear(data["x"], data["weight"], 2, 3, 4), data["bias"],
                        dtype=np.float32)
        result = softmax(logits)
    elif graph == "attention":
        probabilities = softmax(linear(data["q"], data["k"], 2, 3, 4))
        result = np.zeros((2, 2), dtype=np.float32)
        for i in range(4):
            product = np.multiply(probabilities[:, i, None],
                                  data["v"].reshape(4, 2)[None, i, :], dtype=np.float32)
            result = np.add(result, product, dtype=np.float32)
    else:
        logits = np.add(linear(data["x"], data["weight"], 2, 2, 2), data["bias"], dtype=np.float32)
        logits = np.multiply(np.maximum(logits, np.float32(0)), data["scale"].reshape(2, 2),
                             dtype=np.float32)
        probabilities = softmax(logits)
        result = np.add(probabilities[:, 0], probabilities[:, 1], dtype=np.float32)
    return result.ravel()
...
def test_independent_numpy_oracle_and_exact_public_shapes(graph, case):
    ...
    if graph in ("softmax", "classifier"):
...
def test_context_is_inert_bounded_and_covers_every_exclusion(context):
    ...
    for graph in consumer.GRAPHS:
        ...
        scalars += 4 * outputs
    assert rejected == counts["rejected_cases"] == 108
    assert sentinels == counts["sentinel_checks"] == 544
    assert scalars == counts["scalar_checks"] == 80
