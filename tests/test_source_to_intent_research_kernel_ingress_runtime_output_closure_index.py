...
def test_kernel_ingress_runtime_output_closure_index_report_shape() -> None:
    ...
    assert report["output_closure_index_contract"] == (
        SOURCE_TO_INTENT_RESEARCH_KERNEL_INGRESS_RUNTIME_OUTPUT_CLOSURE_INDEX_CONTRACT
    )
    assert report["output_closure_schema_version"] == (
        RUNTIME_EXECUTION_OUTPUT_CLOSURE_REPORT_SCHEMA_VERSION
    )
    assert report["output_closure_contract"] == (
        RUNTIME_EXECUTION_OUTPUT_CLOSURE_CONTRACT
    )
    assert report["output_closure_policy_id"] == (
        RUNTIME_EXECUTION_OUTPUT_CLOSURE_POLICY_ID
    )
    ...
    assert mvp_case["output_closure_check_count"] == 2
    assert mvp_case["output_count"] == 1
...
def test_kernel_ingress_runtime_output_closure_index_contract_rejects_drift(
    tamper_key: str,
    tamper_value: object,
    error: str,
) -> None:
...
def test_kernel_ingress_runtime_output_closure_index_rejects_case_drift() -> None:
    report = build_kernel_ingress_runtime_output_closure_index_report()
    cases = report["cases"]
...
def test_kernel_ingress_runtime_output_closure_index_matches_golden() -> None:
    assert build_report() == GOLDEN_PATH.read_text(encoding="utf-8")


def test_kernel_ingress_runtime_output_closure_index_example_runs() -> None:
    completed = subprocess.run(
        [
            sys.executable,
            (
                "examples/"
                "source_to_intent_research_kernel_ingress_runtime_output_closure_index.py"
            ),
        ],
        check=True,
        capture_output=True,
    ...
    assert '"output_closure_contract"' in completed.stdout
    assert '"output_closure_report_digest"' in completed.stdout
...
def test_kernel_ingress_runtime_output_closure_index_schema_declares_contract() -> None:
    schema = json.loads(SCHEMA_PATH.read_text(encoding="utf-8"))

    assert schema["properties"]["schema_version"]["const"] == (
        SOURCE_TO_INTENT_RESEARCH_KERNEL_INGRESS_RUNTIME_OUTPUT_CLOSURE_INDEX_REPORT_SCHEMA_VERSION
    )
    assert schema["properties"]["output_closure_index_contract"]["const"] == (
        SOURCE_TO_INTENT_RESEARCH_KERNEL_INGRESS_RUNTIME_OUTPUT_CLOSURE_INDEX_CONTRACT
    )
    assert schema["properties"]["output_closure_contract"]["const"] == (
        RUNTIME_EXECUTION_OUTPUT_CLOSURE_CONTRACT
    )
    assert schema["properties"]["output_closure_schema_version"]["const"] == (
        RUNTIME_EXECUTION_OUTPUT_CLOSURE_REPORT_SCHEMA_VERSION
    )
    assert schema["$defs"]["case"]["additionalProperties"] is False
    assert "runtime_evidence_bundle_index_digest" in schema["required"]


def test_kernel_ingress_runtime_output_closure_index_is_documented_and_in_ci() -> None:
    example_path = (
        "examples/"
        "source_to_intent_research_kernel_ingress_runtime_output_closure_index.py"
    )
