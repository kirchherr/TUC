...
def test_admitting_source_ingestion_rfc_example_runs() -> None:
    ...
    assert "ci_replay_for_admitted_slice" not in completed.stdout
    assert "source_to_intent_plain_data_output_golden_for_admitted_slice" not in completed.stdout
    assert "source_free_diagnostics_admission_tests" not in completed.stdout
