"""Pure checks for the fixed installed Linear native harness; no native execution."""
...
def consumer(tmp_path, monkeypatch):
    folder = Path(__file__).resolve().parents[1] / "integration/bounded_linear_native"
    spec = importlib.util.spec_from_file_location("linear_native_consumer", folder / "consumer.py")
...
def _replay(manifest, values):
    ...
    for event in manifest["events"]:
        if event["kind"] == "bind_input":
            ...
            pytest.fail("CPU-only Linear must not copy across targets")
    return {item["public_name"]: public[item["tensor"]] for item in manifest["public_outputs"]}


@pytest.mark.parametrize("graph", ["nonsquare", "singleton", "mixed", "mlp"])
...
def test_numpy_replay_matches_independent_scalar_oracle_and_native_bits(consumer, graph, case):
    ...
    assert any(op["kind"] == "matmul_rhs_transposed" for op in manifest["operations"])
...
def test_nonsquare_oracle_detects_wrong_weight_orientation(consumer):
    x = consumer.fixed_inputs("nonsquare", 0)["x"]
    w = consumer.fixed_inputs("nonsquare", 0)["weight"]
    wrong = (np.array(x, dtype=np.float32).reshape(2, 3) @
             np.array(w, dtype=np.float32).reshape(3, 4))
    right = consumer.reference_outputs("nonsquare", 0)["scores"]
    assert tuple(wrong.flatten()) != right
    assert consumer.reference_outputs("singleton", 0)["scores"] == (1.875,)
...
def test_fixed_counters_isolation_and_fault_protocol_are_explicit(consumer):
    ...
    assert report["expected_baseline"] == {
        "schema_version": "tuc.bounded_linear_native_observation.v0",
        "binding_digest": report["binding_digest"], "fault": 0,
        "status": "PASS", "reason": "none", "case_runs": 16, "entrypoint_calls": 132,
        "scalar_checks": 56, "published_outputs": 16, "rejected_cases": 116,
        "sentinel_checks": 406,
    }
    assert "float data[6][14], original[6][14], output[10]" in files["worker.c"]
...
def test_synthetic_complete_receipts_accept_and_stale_binding_rejects(consumer):
    ...
    assert record["schema_version"] == "tuc.bounded_linear_native_record.v0"
    assert len(record["observations"]) == 10
    assert record["observation_scope"] == "four_fixed_linear_cpu_graphs"
...
def test_evidence_coverage_context_identity_and_unexpected_failure_reject(consumer, change):
    ...
    if change == "missing":
        ...
        (path / "nonsquare-entrypoint.c").write_text("wrong code")
    elif change == "image":
