...
SCHEMA_PATH = Path("schemas/runtime_layout_conversion_trace_replay_verifier_report.v0.schema.json")


def test_runtime_layout_conversion_trace_replay_verifier_passes() -> None:
    report = build_layout_conversion_trace_replay_verifier_report()

    assert report.replay_contract == (RUNTIME_LAYOUT_CONVERSION_TRACE_REPLAY_VERIFIER_CONTRACT)
    assert report.replay_mode == RUNTIME_LAYOUT_CONVERSION_TRACE_REPLAY_VERIFIER_REPLAY_MODE
    assert report.input_policy == (RUNTIME_LAYOUT_CONVERSION_TRACE_REPLAY_VERIFIER_INPUT_POLICY)
    ...
    assert assert_runtime_layout_conversion_trace_replay_verifier(report) is report


def test_runtime_layout_conversion_trace_replay_verifier_dump_matches_golden() -> None:
    assert build_report() == GOLDEN_PATH.read_text(encoding="utf-8")


def test_runtime_layout_conversion_trace_replay_verifier_example_runs() -> None:
    completed = subprocess.run(
        [sys.executable, "examples/runtime_layout_conversion_trace_replay_verifier.py"],
        check=True,
        capture_output=True,
        text=True,
    )
    ...
    assert RUNTIME_LAYOUT_CONVERSION_TRACE_REPLAY_VERIFIER_CONTRACT in completed.stdout
...
def test_runtime_layout_conversion_trace_replay_verifier_detects_forged_evidence_digest() -> None:
    evidence_text, trace_index_text = _current_serialized_reports()
    trace_index = json.loads(trace_index_text)
    trace_index["source_layout_conversion_evidence_digest"] = "sha256:" + "1" * 64
    forged_trace_index_text = json.dumps(trace_index, indent=2, sort_keys=True) + "\n"

    report = build_runtime_layout_conversion_trace_replay_verifier_report(
        evidence_text,
    ...
    assert {issue.issue_code for issue in report.issues} == {
        "layout_conversion_evidence_digest_replayed_mismatch",
    }
    with pytest.raises(AssertionError, match="layout_conversion_evidence_digest"):
        assert_runtime_layout_conversion_trace_replay_verifier(report)


def test_runtime_layout_conversion_trace_replay_verifier_detects_record_drift() -> None:
    evidence_text, trace_index_text = _current_serialized_reports()
    trace_index = json.loads(trace_index_text)
    ...
    assert not report.passed
    assert {issue.issue_code for issue in report.issues} == {
        "conversion_metadata_digest_replayed_mismatch",
    }


def test_runtime_layout_conversion_trace_replay_verifier_rejects_source_or_raw_values() -> None:
    evidence_text, trace_index_text = _current_serialized_reports()

    with pytest.raises(ValueError, match="forbidden layout trace replay"):
        build_runtime_layout_conversion_trace_replay_verifier_report(
...
def test_runtime_layout_conversion_trace_replay_verifier_rejects_non_json_report() -> None:
...
def test_runtime_layout_conversion_trace_replay_verifier_schema_matches_contract() -> None:
    schema = _load_schema()
...
def test_runtime_layout_conversion_trace_replay_verifier_golden_matches_schema_shape() -> None:
    schema = _load_schema()
    ...
    assert golden["replay_contract"] == (RUNTIME_LAYOUT_CONVERSION_TRACE_REPLAY_VERIFIER_CONTRACT)
    assert golden["check_count"] == len(golden["checks"]) == 6
    assert golden["passed"] is True
    assert golden["issues"] == []


def test_runtime_layout_conversion_trace_replay_verifier_is_referenced() -> None:
    example_path = "examples/runtime_layout_conversion_trace_replay_verifier.py"
    schema_path = "schemas/runtime_layout_conversion_trace_replay_verifier_report.v0.schema.json"
    ...
    for path in (
        ...
        Path("docs/RUNTIME_LAYOUT_CONVERSION_TRACE_REPLAY_VERIFIER.md"),
    ):
