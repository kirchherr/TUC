...
GOLDEN_PATH = Path("tests/golden/runtime_backend_equivalence_transfer_binding/current_report.json")
SCHEMA_PATH = Path("schemas/runtime_backend_equivalence_transfer_binding_report.v0.schema.json")


def test_runtime_backend_equivalence_transfer_binding_passes() -> None:
    report = build_backend_equivalence_transfer_binding_report()

    assert report.binding_contract == RUNTIME_BACKEND_EQUIVALENCE_TRANSFER_BINDING_CONTRACT
    assert report.binding_mode == RUNTIME_BACKEND_EQUIVALENCE_TRANSFER_BINDING_MODE
    assert report.input_policy == RUNTIME_BACKEND_EQUIVALENCE_TRANSFER_BINDING_INPUT_POLICY
    ...
    assert report.transfer_replay_check_count == 6
    ...
    assert assert_runtime_backend_equivalence_transfer_binding(report) is report


def test_runtime_backend_equivalence_transfer_binding_dump_matches_golden() -> None:
    assert build_report() == GOLDEN_PATH.read_text(encoding="utf-8")


def test_runtime_backend_equivalence_transfer_binding_example_runs() -> None:
    completed = subprocess.run(
        [sys.executable, "examples/runtime_backend_equivalence_transfer_binding.py"],
        check=True,
        capture_output=True,
        text=True,
    )
    ...
    assert RUNTIME_BACKEND_EQUIVALENCE_TRANSFER_BINDING_CONTRACT in completed.stdout
    assert "metadata_digest_binding_only" in completed.stdout
    assert "runtime_transfer_trace_replay_verifier" in completed.stdout
    assert "transfer_backend_boundary" in completed.stdout
    assert "raw_tensor_value" not in completed.stdout
    assert "python_source" not in completed.stdout
    assert "runtime_handle" not in completed.stdout


def test_runtime_backend_equivalence_transfer_binding_detects_graph_mismatch() -> None:
    ...
    report = build_runtime_backend_equivalence_transfer_binding_report(
        equivalence_text,
        forged_replay_text,
    )
...
def test_runtime_backend_equivalence_transfer_binding_detects_single_candidate_backend() -> None:
    equivalence_text, replay_text = _current_serialized_reports()
...
def test_runtime_backend_equivalence_transfer_binding_rejects_source_or_raw_values() -> None:
    equivalence_text, replay_text = _current_serialized_reports()

    with pytest.raises(ValueError, match="forbidden backend transfer binding"):
        build_runtime_backend_equivalence_transfer_binding_report(
...
def test_runtime_backend_equivalence_transfer_binding_rejects_non_json_report() -> None:
...
def test_runtime_backend_equivalence_transfer_binding_rejects_missing_checks() -> None:
    report = build_backend_equivalence_transfer_binding_report()
...
def test_runtime_backend_equivalence_transfer_binding_schema_matches_contract() -> None:
    schema = _load_schema()
    ...
    assert schema["properties"]["transfer_replay_check_count"]["const"] == 6
...
def test_runtime_backend_equivalence_transfer_binding_golden_matches_schema_shape() -> None:
    schema = _load_schema()
    ...
    assert golden["binding_contract"] == (RUNTIME_BACKEND_EQUIVALENCE_TRANSFER_BINDING_CONTRACT)
    assert golden["check_count"] == len(golden["checks"]) == 8
    assert golden["transfer_replay_check_count"] == 6
    assert golden["candidate_backend_count"] == 2
    assert golden["passed"] is True
    assert golden["issues"] == []


def test_runtime_backend_equivalence_transfer_binding_is_referenced() -> None:
    example_path = "examples/runtime_backend_equivalence_transfer_binding.py"
    schema_path = "schemas/runtime_backend_equivalence_transfer_binding_report.v0.schema.json"
