...
GOLDEN_PATH = Path("tests/golden/runtime_backend_equivalence/vector_sim_report.json")


def test_runtime_vector_backend_equivalence_passes() -> None:
    report = build_vector_backend_equivalence_report()

    assert report.passed
    assert report.issues == ()
    assert report.graph_name == "runtime_vector_backend_equivalence"
    assert report.baseline_run_id == "reference_cpu"
    assert report.candidate_run_id == "vector_sim"
...
def test_runtime_vector_backend_equivalence_dump_matches_golden() -> None:
    assert dump_runtime_backend_equivalence_report(
        build_vector_backend_equivalence_report()
    ) == (GOLDEN_PATH.read_text(encoding="utf-8").rstrip("\n") + "\n")


def test_runtime_vector_backend_equivalence_example_runs() -> None:
    completed = subprocess.run(
        [sys.executable, "examples/runtime_vector_backend_equivalence.py"],
        check=True,
        capture_output=True,
        text=True,
    )
