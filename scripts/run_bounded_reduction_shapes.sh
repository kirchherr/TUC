...
python3 examples/bounded_reduction_shapes.py > /dev/null
umask 077
mkdir -p tmp
evidence_dir=$(mktemp -d "tmp/reduction-shape-$target.XXXXXXXX")
container_name="tuc-shape-$target-$$"
image_name="tuc-shape-$target-$$"
cleanup() { docker rm -f "$container_name" > /dev/null 2>&1 || true; }
trap cleanup EXIT
trap 'exit 1' HUP INT TERM
timeout 600s docker build --network=none --target "$target-runtime" \
    -t "$image_name:runtime" docker/reduction-shapes
...
python3 examples/bounded_reduction_shapes.py --target "$target" --preflight \
    --validate "$evidence_dir/preflight.json"
run_worker "$image_id" /opt/tuc/proof --execute > "$evidence_dir/execution.json"
python3 examples/bounded_reduction_shapes.py --target "$target" \
    --validate "$evidence_dir/execution.json" --image-id "$image_id" > "$evidence_dir/record.json"
for mutation in missing-sum wrong-stride incomplete-coverage; do
    result=0
...
if [ "$target" = c11 ]; then
    timeout 600s docker build --network=none --target c11-sanitizer \
        -t "$image_name:sanitizer" docker/reduction-shapes
    ...
    python3 examples/bounded_reduction_shapes.py --target c11 --validate "$evidence_dir/sanitized.json"
fi
printf 'Odd-shape %s: PASS, 20 vectors + replay, three wrong-code probes rejected.\n' "$target"
